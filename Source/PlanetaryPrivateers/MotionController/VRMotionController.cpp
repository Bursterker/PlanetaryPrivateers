...
AVRMotionController::AVRMotionController()
{
	...
	m_pControllerMesh = CreateDefaultSubobject<USkeletalMeshComponent>(TEXT("SK_Hand"));
	m_pControllerMesh->SetupAttachment(m_pHandOrigin);
	m_pControllerMesh->SetCollisionProfileName("NoCollision");

	// TODO
	// > Test locally tracked off
	// >> parent this to owning pawn
...
{
	if (GEngine) GEngine->AddOnScreenDebugMessage(-1, 3.f, FColor::Blue, FString::Printf(TEXT("VRMotionController.cpp::BeginPlay --> The beginPlay is called!")));
	if (GEngine && !m_pControllerMesh) GEngine->AddOnScreenDebugMessage(-1, 10.f, FColor::Red, FString::Printf(TEXT("VRMotionController.cpp::BeginPlay --> The m_pControllerMesh Is null")));

	Super::BeginPlay();
	if (GetNetMode() == NM_DedicatedServer)
	{
		if (GEngine)GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Cyan, TEXT("Spawned on Server"));
	}
	else
	{
		if (GEngine)GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Cyan, TEXT("Spawned on client"));
	}
...
{
	TArray<IMotionController*> tControllers{ IModularFeatures::Get().GetModularFeatureImplementations<IMotionController>(IMotionController::GetModularFeatureName()) };
	...
	{
		if (GEngine)GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Red, TEXT("Failed to retrieve Motion Controller Device Interface. << AVRMotionController::BeginPlay()"));
	}
}
...
void AVRMotionController::Tick(float DeltaTime)
{
	...
	case ETrackingMode::Tracked:
	{
		...
		if (m_bTrackingEnabled)
		{
			...
			if (m_pController)
			{
				...
				m_pController->GetControllerOrientationAndPosition(0, motionSourceName, trackedRot, trackedLoc, meterInUnrealUnits);
	...
	case ETrackingMode::Replicated:
	{
		...
		if (false && m_ControllerHand == EControllerHand::Left)
		{
			...
			DrawDebugSphere(GetWorld(), m_pHandOrigin->GetComponentLocation(), 15.f, 32, FColor::Purple, false, -1.f, ESceneDepthPriorityGroup::SDPG_Foreground);
			
			FVector targetLocation{};
			USceneComponent* pParentCmp = m_pHandOrigin->GetAttachParent();
			if (pParentCmp)
			{
				targetLocation = { pParentCmp->GetComponentTransform().TransformPosition(m_RepTransformTarget.GetLocation()) };
			}
			else
			{
		...
		if (pAttachScnCmp)
		{
			...
			//
			//FTransform currentRelativeTransform = originTransform.Inverse() * GetActorTransform();

			FTransform currentTransform = GetActorTransform();
			FTransform originTransform = pAttachScnCmp->GetComponentTransform();
			FTransform targetTransform{ m_RepTransformTarget * originTransform };
			
			FVector lerpedLocation{ FMath::Lerp(currentTransform.GetLocation(), targetTransform.GetLocation(), alpha) };
			FQuat lerpedRotation{ FMath::Lerp(currentTransform.GetRotation(), targetTransform.GetRotation(), alpha) };
			FVector lerpedScale{ FMath::Lerp(currentTransform.GetScale3D(),targetTransform.GetScale3D(),alpha) };

			SetActorTransform(FTransform(lerpedRotation, lerpedLocation, lerpedScale), false, nullptr, ETeleportType::ResetPhysics);
...
void AVRMotionController::SetTrackingHand(EControllerHand hand)
{
	...
	{
		m_pControllerMesh->SetRelativeScale3D(FVector(m_pControllerMesh->RelativeScale3D.X, m_pControllerMesh->RelativeScale3D.Y , m_pControllerMesh->RelativeScale3D.Z * -1));
	}
}
...
// ===================================================
void AVRMotionController::Server_BroadcastTransform_Implementation(FTransform replicatedTransform)
{
	Multicast_ReplicateTransform(replicatedTransform);
}

bool AVRMotionController::Server_BroadcastTransform_Validate(FTransform replicatedTransform)
{
	return true;
}

// ===================================================
void AVRMotionController::Multicast_ReplicateTransform_Implementation(FTransform replicatedTransform)
{
...
// ===================================================
bool AVRMotionController::Multicast_ReplicateTransform_Validate(FTransform replicatedTransform)
{
	return true;
}